...
const validateAuthorSchema = ajv.compile(
  loadJson(join(SCHEMAS_DIR, 'author.json'))
);
...
function loadAuthor(github) {
  ...
  try {
    ...
    return {
      ok: false,
      error: `authors/${github}.json パース失敗: ${e.message}`,
    };
  }
  // Schema 検証
  if (!validateAuthorSchema(record)) {
    return {
      ok: false,
      error: `authors/${github}.json schema 違反: ${formatAjvErrors(validateAuthorSchema.errors)}`,
    };
  }
  ...
  if (record.github !== github) {
    return {
      ok: false,
      error: `authors/${github}.json の github フィールド (${record.github}) はファイル名と一致する必要があります`,
    };
  }
...
function verifySignature(entry, authorRecord) {
  ...
  if (currentKid === entry.author_pubkey_id) {
    ...
    return {
      ok: false,
      error: `key_id ${entry.author_pubkey_id} が著者鍵と一致しません`,
    };
  }
...
function checkPackFile(entry) {
  ...
  if (stat.size > MAX_PACK_BYTES) {
    return {
      ok: false,
      error: `${path} のサイズ ${stat.size} が 50MB を超えています`,
    };
  }
  ...
  if (typeof entry.size_bytes === 'number' && entry.size_bytes !== stat.size) {
    return {
      ok: false,
      error: `entry.size_bytes (${entry.size_bytes}) が実ファイル (${stat.size}) と一致しません`,
    };
  }
  ...
  if (sha.toLowerCase() !== entry.sha256.toLowerCase()) {
    return {
      ok: false,
      error: `SHA-256 不一致 expected=${entry.sha256} actual=${sha}`,
    };
  }
...
async function checkMalwareVirusTotal(sha, apiKey) {
  ...
  try {
    ...
    logWarn(
      `VirusTotal: ネットワークエラー (${e.message})。ローカル DB のみで継続。`
    );
...
async function main() {
  ...
  if (vtApiKey) {
    ...
    console.log(
      'VirusTotal API: 未設定 — ローカル malware-hashes.txt のみで照合'
    );
  }
  ...
  for (const o of orphans) {
    logErr(
      `themes/${o} に対応する entries/*.json が存在しません (孤立ファイル)`
    );
  }
  ...
  for (const file of targets) {
    ...
    if (!validateEntrySchema(entry)) {
      logErr(
        `${file}: schema 違反: ${formatAjvErrors(validateEntrySchema.errors)}`
      );
      errors++;
      continue;
    }
    ...
    //    CI 側でも担保する (easy-cursor-swap 側で同一不変条件を強制しているが、二重に守る)。
    ...
    if (previewCount === 0) {
      logErr(
        `${entry.id}: .cursorpack 内にプレビューが見つかりません — Arrow.png は必須です`
      );
      errors++;
      continue;
    }
    ...
    const nameLabel =
      typeof entry.name === 'string'
        ? entry.name
        : (entry.name?.en ?? entry.name?.ja ?? '?');
    console.log(
      `  OK: ${nameLabel} (${entry.version}) ${pack.size}B sha=${pack.sha.slice(0, 12)}...${vtLabel}`
    );
  }
